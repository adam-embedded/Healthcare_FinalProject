...
void Logger::log(Severity severity, const char *msg) noexcept {
    ...
    if (severity <= Severity::kWARNING) {//kWARNING //kVERBOSE
...
bool Engine::build(std::string onnxModelPath, std::string deployFile) {
    ...
    auto explicitBatch = 0U << static_cast<uint32_t>(NetworkDefinitionCreationFlag::kEXPLICIT_BATCH);
    ...
    nvinfer1::DataType modeDataType = nvinfer1::DataType::kFLOAT; // data type
    ...
    const nvcaffeparser1::IBlobNameToTensor* blobNameToTensor = parser->parse(deployFile.c_str(),onnxModelPath.c_str(),*network, modeDataType);
...
//            checkCudaErrorCode(cudaMallocAsync(&m_buffers[i], m_options.maxBatchSize * inputBindingDims.d[1] * inputBindingDims.d[2] * inputBindingDims.d[3] * sizeof(float), stream));
            checkCudaErrorCode(cudaMalloc(&m_buffers[i], m_options.maxBatchSize * inputBindingDims.d[1] * inputBindingDims.d[2] * inputBindingDims.d[3] * sizeof(float)));
...
//            checkCudaErrorCode(cudaMallocAsync(&m_buffers[i], outputLenFloat * m_options.maxBatchSize * sizeof(float), stream));
            checkCudaErrorCode(cudaMalloc(&m_buffers[i], outputLenFloat * m_options.maxBatchSize * sizeof(float)));
        }
    }
