...
std::vector<Object> YoloV8::detectObjects(const cv::Mat &inputImgBGR, const bool pose) {
    ...
    return postprocess(featureVector, pose);
}
...
std::vector<Object> YoloV8::postprocess(std::vector<float> &featureVector, const bool pose) {
    ...
    for (int i = 0; i < numAnchors; i++) {
        ...
        if (score > PROBABILITY_THRESHOLD) {
            ...
            //if (!pose) int label = maxSPtr - scoresPtr;
            ...
            for (int k = 0; k < 17; k++) {
                float kps_x = (*(kps_ptr + 3 * k)) * m_ratio;
                float kps_y = (*(kps_ptr + 3 * k + 1)) * m_ratio;
                float kps_s = *(kps_ptr + 3 * k + 2);
                kps_x = std::clamp(kps_x, 0.f, m_imgWidth);
                kps_y = std::clamp(kps_y, 0.f, m_imgHeight);
                kps.push_back(kps_x);
                kps.push_back(kps_y);
                kps.push_back(kps_s);
            }
            ...
            // Check if pose of not
            pose ? labels.push_back(0) : labels.push_back(maxSPtr - scoresPtr);
