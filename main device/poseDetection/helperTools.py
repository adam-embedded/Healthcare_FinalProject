...
class Classifier(object):
  ...
  def classify_pose(self, person: Person) -> List[Category]:
    ...
    # of the TFLite model.
    input_tensor = [
      [keypoint.coordinate.y, keypoint.coordinate.x, keypoint.score] for keypoint in person.keypoints]
...
class Movenet(object):
  ...
  def _run_detector(
    ...
    for idx in range(len(BodyPart)):  # for (int i = 0; i < num_keypoints; ++i) {
